...
export class GetjobService {
  ...
  createNewCategory(data: String): Observable<String> {
    return this.http.post<String>(this.url + "admin/addcategory", data);
  }
  createNewDesignation(data: genreData): Observable<genreData> {
    return this.http.post<genreData>(this.url + "admin/adddesignation", data);
  }
}
